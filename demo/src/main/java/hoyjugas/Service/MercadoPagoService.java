...
public class MercadoPagoService {
    ...
    public String createPreference(Booking booking, BigDecimal amount)
            throws MPException, MPApiException {
        PreferenceClient client = new PreferenceClient();

        PreferenceItemRequest item = PreferenceItemRequest.builder()
                .title("Reserva " + booking.getSpace().getName())
                .description("Turno " + booking.getBookingNumber())
                .quantity(1)
                .unitPrice(amount)
                .currencyId("ARS")
                .build();
    ...
    public Payment getPayment(String paymentId) {//dejar asi por ahora
        try {
